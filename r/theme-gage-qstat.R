...
xml <- read_xml(file.path(config$data_dir, "sciencebase", theme_id, "all_gage_flow_stats.xml"))
...
df_dataset <- read_csv(file.path(config$data_dir, "sciencebase", theme_id, "all_gage_flow_stats.csv"), col_types = cols(
  .default = col_double(),
...
out_dataset <- df_dataset %>% 
  ...
  select(id = site_no, decade, df_vars$id)
stopifnot(
  out_dataset %>% 
...
for (i in 1:nrow(df_feature)) {
  feature_data <- list(
    id = df_feature$id[[i]],
    values = as.list(df_feature$values[[i]])
  )
